...
import { getPreviewParagraphs } from "@/lib/accessControl";
...
export function PropFirmOneWeekGuide({
  hasFullAccess,
  requiredTier,
}: PropFirmOneWeekGuideProps) {
  const guide = PROP_FIRM_ONE_WEEK_GUIDE;

  const previewSource = [
    guide.description,
    ...guide.overview,
    ...guide.consistencyRules.slice(0, 2),
    ...guide.dailyPlans.slice(0, 2).map(
      (d) =>
        `Day ${d.day} — ${d.title}: ${d.focus}. ${d.tasks[0]}`
    ),
  ];
    ...
    <div className="space-y-10">
      ...
      <section className="rounded-xl border border-cyan-accent/20 bg-cyan-accent/5 p-6">
        ...
        <ul className="mt-4 space-y-3">
          {(hasFullAccess ? guide.overview : preview.paragraphs).map((line) => (
                ...
                >
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    ...
                    <Badge variant="warning">
                      Max risk: {day.maxRiskPercent}%/trade
                    </Badge>
                  </div>
    ...
    <div className="space-y-10">
      <header>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          {!hasFullAccess && (
            <Badge variant="warning">Preview · {requiredTier}</Badge>
          )}
        </div>
      ...
      {hasFullAccess ? (
        ...
        <>
          <FadedPreview>{bodyContent}</FadedPreview>
          <FreemiumTierCTA
            resourceType="guide"
            resourceId={guide.slug}
