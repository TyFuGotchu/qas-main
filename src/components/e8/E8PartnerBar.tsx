...
export function E8PartnerBar({ href }: { href?: string }) {
  return (
    ...
    >
      <span className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#C8ACFF] sm:text-[11px]">
        {E8_PARTNER_LINE}
      </span>
      <span className="hidden h-3 w-px bg-[#B794FF]/40 sm:block" />
      <span className="font-mono text-[10px] text-[#C8ACFF]/80 sm:text-[11px]">{E8_BAR_LINE}</span>
    </Link>
  );
}
